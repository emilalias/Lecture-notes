...
Soit $A\in \mathbf{C}^{n\times n}$ et soient $J$ une forme normale de Jordan de $A$, $P$ la matrice de passage associée ($A=PJP^{-1}$).
\begin{enumerate}
...
Soit $A\in \mathbf{C}^{n\times n}$ une matrice et $J\in \mathbf{C}^{n\times n}$ sa forme normale de Jordan. Appelons un bloc de Jordan ayant $\lambda$ sur sa diagonale un bloc \emph{associe} a $\lambda$. Alors
\begin{enumerate}
...
\end{proof}
\begin{exercise}[Exo etoile]
En utilisant la forme normale de Jordan montrer que toute matrice $A\in \mathbf{C}^{n\times n}$ est semblable a sa transposée $A^{\mathsf{T}}$.
\end{exercise}
\begin{proof}[Solution]
Par le théorème de Jordan, il existe $P\in \mathrm{GL}_{n}(\mathbf{C})$ telle que $A=P^{-1}JP$ ou $J$ est la forme normale de Jordan de $A$. Alors 
$$
...
\end{pmatrix}.
$$
On définit $C(k):=\begin{pmatrix} &  & 1 \\  & \ddots \\ 1 \\ \end{pmatrix}\in \mathbf{C}^{k\times k}$ et par un calcul, on voit que
$$
J(\lambda,k)=C(k)J(\lambda,k)^{\mathsf{T}}C(k).
$$
En plus $C(k)^{-1}=C(k)$. Donc on construit la matrice $\tilde{C}\in \mathbf{C}^{n\times n}$ en prenant $J$ et en remplaçant chaque bloc $(J(\lambda,k))$ de Jordan par un bloc $C(k)$. On a
$$
