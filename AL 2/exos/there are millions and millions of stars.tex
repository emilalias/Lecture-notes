...
\begin{exercise}[Exo etoile]
Soit $A\in \mathbf{R}^{n\times n}$ une matrice symétrique.
...
On prend $I=\{ \ell_{1},\dots,\ell _{k} \}\subseteq[n]$. On définit $f:\mathbf{R}^{|I|}\to \mathbf{R}^{n}$ tel que $f(x)_{\ell _{j}}=x_{j}$ et $f(x)_{j}=0$ sinon. Pour $y\in \mathbf{R}^{|I|}$ on a $f(y)^{\mathsf{T}}Af(y)=y^{\mathsf{T}}A_{I}y\ge 0$ donc $A_{I}$ est semi-definie positive.
...
\begin{exercise}[Exo plus]
Soit $A=(a_{ij})\in \mathbf{R}^{n\times n}$ une matrice symétrique semi-definie positive.
\begin{enumerate}
...
\item Le théorème spectral donne une base orthonormale $\{ u_{i} \}$ forme des vecteurs propres de $A$. Pour $x \in \mathbf{R}^{n}$, on l'ecrit comme $x=\sum_{i=1}^{n}\alpha _{i}u_{i}$ alors
$$
...
Soit $A\in \mathbf{R}^{n\times n}$ symétrique et $\lambda_{1}\ge\dots\ge\lambda _{n}$ les valeurs propres. Montrer que pour tout $i=1,\dots,n$,
$$
...
\end{proof}
\begin{exercise}[Exo etoile]
Soit $A\in \mathbf{R}^{n\times n}$ une matrice symétrique et soit
$$
A=U\begin{pmatrix}
\lambda_{1} &  &  \\
 & \ddots &  \\
...
$$
une factorisation de $A$ telle que $U=(u_{1},\dots,u_{n})\in \mathbf{R}^{n\times n}$ est orthogonale et $\lambda_{1}\ge\cdots\ge\lambda _{n}$. Pour $1\le\ell<n$, montrer qu'on a 
\begin{equation}
...
\end{exercise}
\begin{proof}[Solution]
On procède par double inégalité. Prenons $x \in S^{n-1}$, on écrit $x=\sum _{i=1}^{n}\alpha _{i}u_{i}$ ou $\alpha _{i}\in \mathbf{R}$. Si $x\perp u_{1},\dots,x\perp u_{\ell}$, alors comme $\{ u_{1},\dots,u_{n} \}$ forme une base orthonormale de $\mathbf{R}^{n}$, on observe que $\alpha_{1}=\dots=\alpha _{\ell}=0$. Alors $x=\sum_{i=\ell +1}^{n}\alpha _{i}u_{i}$. Comme $\lVert x \rVert=1$, on a que $\sum_{i=\ell +1}^{n}\alpha _{i}^{2}=1$. On calcule
\begin{align*}
...
\chapter{Serie 10}
\begin{exercise}[Exo plus]
Soient $A\in \mathbf{R}^{m\times n}$ et $b\in \mathbf{R}^{m}$. On considère l'application
$$
f:\mathbf{R}^{n}\to \mathbf{R}:x\mapsto f(x)=\lVert Ax- b\rVert^{2} .
$$
...
\end{align*}
Prenons $h\in \mathbf{R}^{n}$ et calculons la différentielle $D_{f,x}(h)$ :
\begin{align*}
...
Alors 
$$
\mathrm{grad}(f)=2A^{\mathsf{T}}Ax-2A^{\mathsf{T}}b\in \mathbf{R}^{n}.
$$
...
\begin{exercise}[Exo etoile]
Soient $a_{1},\dots,a_{m}\in \mathbf{R}^{n}$. Pour $z\in \mathbf{R}^{n}$ et $v\in S^{n-1}$, on considère la droite $L_{z,v}$ définie par
$$
L_{z,v}=\{ z+\lambda v:\lambda \in \mathbf{R} \}=z+\mathrm{span}\{ v \}.
$$
Soit $d(a_{i},L_{z,v})$ la distance de $a_{i}$ a $L_{z,v}$ pour $i=1,\dots,m$.
\begin{enumerate}
\item Supposons que $\sum_{i=1}^{m}a_{i}=0$. Montrer que $\sum_{i=1}^{m}d(a_{i},L_{z,v})^{2}\ge \sum_{i=1}^{m}d(a_{i},L_{0,v}^{2})$ pour tout $z\in \mathbf{R}^{n}$.
\item Conclure que, étant donnes des points $a_{1},\dots,a_{m},$ on peut trouver un $z\in \mathbf{R}^{n}$ et $v\in S^{n-1}$ tels que
$$
\sum_{i=1}^{m} d(a_{i},L_{z,v})^{2}\le \sum_{i=1}^{m} d(a_{i},L_{z',v'})^{2},
$$
pour tout $z'\in \mathbf{R}^{n}$ et $v'\in S^{n-1}$. En particulier :
\begin{enumerate}
...
\end{align*}
\item Sans supposition sur $\sum _{i}a_{i}$, posons $z:=\frac{1}{m}\sum _{i}a_{i}$. Avec ce $z$, le points $a_{i}-z$ satisfont l'hypothese de 1 (car $\sum _{i}(a_{i}-z)=0$). Pour $v'\in S^{n-1}$ et pour tout $z'\in \mathbf{R}^{n}$, on écrit $z'=z+z''$ avec $z''\in \mathbf{R}^{n}$. On calcule
\begin{align*}
...
Par le cours, on pose $A=(a_{1}-z, \dots , a_{m}-z)^{\mathsf{T}}\in \mathbf{R}^{m\times n}$. Avec ce setup, par le cours on sait que $v$ est donne par un vecteur propre normalise associe a la plus grande valeur propre de $A^{\mathsf{T}}A$.  Pour tout $z'\in \mathbf{R}^{n}$ et pour tout $v'\in S^{n-1}$ on a
\begin{align*}
\sum _{i}d(a_{i},L_{z,v})^{2} & =\sum _{i}d(a_{i}-z,\mathrm{span}\{ v \})^{2}  \\
...
\begin{enumerate}
\item $A=\begin{pmatrix}x & 0 \\ 0 & y\end{pmatrix}$ ou $x,y\in \mathbf{C}$.
\item Une matrice $A\in \mathbf{C}^{n\times n}$ qui satisfait $A^{2}=A$.
\end{enumerate}
\end{exercise}
\begin{proof}[Solution]
\begin{enumerate}
...
\end{proof}
\begin{exercise}[Exo etoile]
Soit $A\in \mathbf{C}^{n\times n}$ inversible. Montrer que $A$ est diagonalisable si et seulement si $A^{m}$ est diagonalisable pour un certain $m\in \mathbf{N}^{*}$.

Donner un contre-exemple lorsque $A$ n'est pas inversible.

\emph{Pour la resolution de cet exercice, vous pouvez admettre le résultat suivant : une matrice $A\in \mathbf{C}^{n\times n}$ est diagonalisable si et seulement si son polynôme minimal ne possède que des racines simples (c-a-d de multiplicité 1). Ce résultat sera démontré a partir du théorème de Jordan, qui constitue l'objectif des deux semaines a venir.}
\end{exercise}
\begin{proof}[Solution]
\begin{description}
...
\chapter{Serie 12}
\begin{exercise}[Exo plus]
Soit $A\in \mathbf{C}^{n\times n}$. Le \emph{polynôme minimal} de $A$ est le polynôme de degré minimal unitaire dans $\mathbf{C}[x]\setminus \{ 0 \}$ qui annule $A$.
\begin{enumerate}
...
\item Soit $p(x)\in \mathbf{C}[x]$ tel que $p(A)=0$. Alors $p(x)=q(x)m_{A}(x)+r(x)$ avec $\deg r <\deg m_{A}$ (division euclidienne). On a
$$
