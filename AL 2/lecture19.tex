...
Etant donnes $k\le m\in \mathbf{N}$ et $a_{1},\dots,a_{n}\in \mathbf{R}^{m}$, on veut trouver $H\le \mathbf{R}^{n}$ de $\dim(H)\le k$ minimisant $\sum _{i=1}^{n}d(a_{i},H)^{2}$ ou $d$ est une distance.

Supposons que $H=\mathrm{span}\{ u_{1},\dots,u_{k} \}$ ou les $u_{i}$ sont orthonormaux. Une projection $\pi$ de $a_{i}$ sur $H$ est définie par
$$
\pi(a)=\sum_{j=1}^{k} \langle a_{i},u_{j} \rangle u_{j}
$$
...
On veut maximiser
$$
\sum_{i=1}^{n} \lVert \pi(a_{i}) \rVert ^{2}.
$$