...
Soit \todo{Lecture 4} $\boldsymbol{x}\in \mathbf{R}^{n}$ et $\delta>0\in \mathbf{R}$:
...
\item Un point d'accumulation si $\forall\delta>0$ on a $B(\boldsymbol{x},\delta)\cap E\setminus \{ \boldsymbol{x} \}\neq \varnothing$
\end{itemize}
\end{itemize}
...
\begin{proof}
Soient $\{ E_{i} \}_{i\in \mathbf{N}}$ ouverts et $F=\bigcap _{i=1}^{k}E_{i}$ et $\boldsymbol{x }\in F$ alors $\boldsymbol{x}\in E_{i}$ pour tous $i$ donc il existe $\delta _{i}>0$ tel que $B(\boldsymbol{x},\delta _{i})\subset E_{i}$ et $B(\boldsymbol{x},\min_{i=1,\dots,k} \delta_{i})\subset F$ alors $F$ est ouvert.
\end{proof}
\begin{remark}
