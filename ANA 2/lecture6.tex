...
Soit $E\subset \mathbf{R}^{m}$ et $\boldsymbol{x}_{0}\in E$ un point d'accumulation de $E$. Soient $f,g:E\to \mathbf{R}$, supposons que $\lim_{ \boldsymbol{x} \to \boldsymbol{x}_{0} }f(\boldsymbol{x})=\ell_{1}$ et $\lim_{ \boldsymbol{x} \to \boldsymbol{x}_{0} }g(\boldsymbol{x})=\ell_{2}$:
\begin{enumerate}
...
Ceci montre que $\lim_{ \boldsymbol{x} \to \boldsymbol{a} }\boldsymbol{f}(\boldsymbol{x})$ existe pour tou $\boldsymbol{a}\in \overline{E}\setminus E$ et on peut construire le prolongement par continuite $\boldsymbol{\tilde{f}}$ sur $E$.
