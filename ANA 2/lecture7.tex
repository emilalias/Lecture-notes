...
\begin{theorem}
Soit \footnote{Lecture 7} $E\subset \mathbf{R}^{n}$ compact non-vide et $f:E\to \mathbf{R}$ continue. Alors $f$ est bornee et atteint ses bornes, c-a-d il exists $\boldsymbol{x}_{M}\in E$ tel que $f(\boldsymbol{x}_{M})=\max_{\boldsymbol{x}\in E}f(\boldsymbol{x})$ et $\boldsymbol{x}_{m}\in E$ tel que $f(\boldsymbol{x}_{m})=\min_{\boldsymbol{x}\in E}f(\boldsymbol{x})$.
\end{theorem}
\begin{proof}
\begin{itemize}
